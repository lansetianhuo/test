...
def inft_info_collect(phy_intf_list):
    ...
    for data_row in phy_intf_list:
        ...
        if 'gei' in data_row[0]:
            ...
            intf_info_dict['入流量'] = data_row[1] + '%'
            intf_info_dict['出流量'] = data_row[2] + '%'
            intf_info_dict['端口描述'] = 'none'
            if len(data_row)> 4:
                intf_info_dict['端口描述'] = ''.join(data_row[4:])
...
def main():
    read_file_path = r'D:\xuexi\cheshiwenjian\中兴MSE流量-新.txt'
    write_file_path = r'D:\xuexi\cheshiwenjian\中兴MSE端口流量-新.csv'
