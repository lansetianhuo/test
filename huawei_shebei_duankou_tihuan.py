...
# -*- encoding: utf-8 -*-
'''
@File    :   huawei_shebei_duankou_tihuan.py
@Time    :   2020/03/03 16:46:59
@Author  :   lanse
@Version :   1.0
@Contact :   @qq.com
@WebSite :
...
可替换端口名内容：
华为设备：所有以“interface+端口名”格式配置中的端口名; 静态路由配置绑定的端口名；静态用户绑定的端口名；
静态BFD绑定的端口名；l2vpn绑定的端口名。

使用说明：
将老、新设备的端口对应关系以每个对应关系为一行的形式存入"端口关系.txt"文件，并把文件路径赋值给duankou_path变量
duankou_path = "D:\\xuexi\\cheshiwenjian\\端口关系.txt"
文件格式如下：
GigabitEthernet1/0/0 GigabitEthernet3/0/0
GigabitEthernet1/0/1 GigabitEthernet3/0/1
GigabitEthernet1/1/3 GigabitEthernet3/1/3
GigabitEthernet1/1/4 GigabitEthernet3/1/4
GigabitEthernet2/0/0 GigabitEthernet4/1/0
GigabitEthernet2/0/1 GigabitEthernet4/1/1
...
def tihuan_duankou(peizhi, duankou_guanxibiao):
    ...
    for peizhi_yuansu in peizhi_liebiao:
        ...
        if str(peizhi_yuansu_tmp).startswith('ip route-static'):
            # 判断元素是否为静态路由配置并修改指定了端口的静态路由中相应端口名
            peizhi_yuansu_tmp = re.split(r'[ .]', peizhi_yuansu_tmp)
            if peizhi_yuansu_tmp[2] == 'vpn-instance':
        ...
        if str(peizhi_yuansu_tmp).startswith('ipv6 route-static'):
            # 判断元素是否为ipv6静态路由配置并修改指定了端口的静态路由中相应端口名
            peizhi_yuansu_tmp = re.split(r'[ .]', peizhi_yuansu_tmp)
            if peizhi_yuansu_tmp[2] == 'vpn-instance':
                if len(peizhi_yuansu_tmp) >= 7:
                    for guanxi in duankou_guanxibiao:
                        if str(peizhi_yuansu_tmp[6]) == guanxi[0]:
                            peizhi_yuansu = peizhi_yuansu.replace(
                                peizhi_yuansu_tmp[6], guanxi[1])
            else:
                if len(peizhi_yuansu_tmp) >= 5:
                    for guanxi in duankou_guanxibiao:
                        if str(peizhi_yuansu_tmp[4]) == guanxi[0]:
                            peizhi_yuansu = peizhi_yuansu.replace(
                                peizhi_yuansu_tmp[4], guanxi[1])
        if str(peizhi_yuansu_tmp).startswith(
                'bfd') and peizhi_yuansu_tmp.find('interface') > 0:
            # 判断元素是否为bfd配置并修改指定了端口的bfd配置中相应端口名
            peizhi_yuansu_tmp = re.split(r'[ .]', peizhi_yuansu_tmp)
            for guanxi in duankou_guanxibiao:
                if str(peizhi_yuansu_tmp[peizhi_yuansu_tmp.index('interface') +
                                         1]) == guanxi[0]:
                    peizhi_yuansu = peizhi_yuansu.replace(
                        peizhi_yuansu_tmp[peizhi_yuansu_tmp.index('interface') +
                                          1], guanxi[1])
        if str(peizhi_yuansu_tmp).startswith(
                'static-user') and peizhi_yuansu_tmp.find('interface') > 0:
                ...
                if str(peizhi_yuansu_tmp[peizhi_yuansu_tmp.index('interface') +
                    ...
                    peizhi_yuansu = peizhi_yuansu.replace(
                        peizhi_yuansu_tmp[peizhi_yuansu_tmp.index('interface') +
                                          1], guanxi[1])
        if str(peizhi_yuansu_tmp).startswith(
                'ccc') and peizhi_yuansu_tmp.find('interface') > 0:
            # 判断元素是否为ccc配置并修改指定了端口的ccc配置中相应端口名
            peizhi_yuansu_tmp = re.split(r'[ .]', peizhi_yuansu_tmp)
            for guanxi in duankou_guanxibiao:
                if str(peizhi_yuansu_tmp[peizhi_yuansu_tmp.index('interface') +
                                         1]) == guanxi[0]:
                    peizhi_yuansu_tmp[peizhi_yuansu_tmp.index('interface') +
                                      1] = guanxi[1]
                if str(peizhi_yuansu_tmp[
                        peizhi_yuansu_tmp.index('out-interface') +
                        1]) == guanxi[0]:
                    peizhi_yuansu_tmp[peizhi_yuansu_tmp.index('out-interface') +
                                      1] = guanxi[1]
                peizhi_yuansu = ' '.join(peizhi_yuansu_tmp)
