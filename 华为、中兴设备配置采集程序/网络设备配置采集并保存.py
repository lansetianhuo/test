...
def huawei_configfile_collect(ip_item, dev_list):
    ...
    try:
        ...
        configfile_col = connect.send_command(
            'display current-configuration',
            strip_prompt=False,
            strip_command=False)
        job_col = connect.send_command_timing('save') + '\n'
        job_col += connect.send_command_timing('y',
                                               strip_prompt=False,
                                               strip_command=False)
        connect.disconnect()
        ...
        log_str += '\n##############################\n' + configfile_col
        logout_file_path = rf'.\采集配置目录{now}\{hostname}-configfile-{now}.txt'

        output_file(logout_file_path, log_str)
        log_str += '\n##############################\n' + job_col
        job_log_path = rf'.\操作日志{now}\{hostname}-log-{now}.txt'

        output_file(job_log_path, log_str)
...
if not os.path.isdir(f'采集配置目录{now}'):
    os.mkdir(f'采集配置目录{now}')
