...
def extract_used_ip(routing_table, usable_ip_set):
    ...
    for route_ip in routing_table:
        for usable_ip in usable_ip_set:
            if route_ip in usable_ip and IP(route_ip) != usable_ip:
...
# 从城域网CR路由原表文件读取数据并纯化存成二维列表，将第一个元素为IP网段的，第二个元素为非静态路由的列表存入col_data_list列表中.
# 并输出城域网CR路由表清理后文件
...
def ip_mask_filter(data_list, ip_mask_len):
    ip_data_list = []
    for row_data in data_list:
        row_data = str(row_data).split(r'/')
        if ip_true(row_data[0]) and len(row_data) > 1:
            if row_data[1] <= ip_mask_len:
                ip_data_list.append(row_data[0] + '/' + row_data[1])
    return ip_data_list
